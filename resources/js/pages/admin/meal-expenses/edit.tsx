...
export default function MealExpenseEdit({ expense, halls }: { expense: any, halls: any[] }) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Edit Expenses',
            href: `/admin/meal-expenses/${expense.id}/edit`,
        },
    ];
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        extra_mutton_charge: expense.extra_mutton_charge || '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/meal-expenses/${expense.id}`);
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Meal Expenses" />
            <div className="p-6 max-w-4xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" onClick={() => window.history.back()}>
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                    <h2 className="text-2xl font-bold">Edit Daily Expenditure</h2>
                </div>
